...
public class Game {
    public static void main(String[] args) {
        ...
        CheckQuestions checkers = new CheckQuestions(); //object om vragen te checken
        gameOn.setKamerquestions(); //voegt de vragen toe aan een arraylist
        checkers.checkquestions(); //activeert de checker

        for (int i = 0; i < gameOn.getQuestionCount(); i++) {
            gameOn.getKamerquestions(i);
            gameOn.SubQuestions(i);
            String text = scanner.nextLine();
            checkers.sendCorrectionMessage(i, text);
            ...
            for(int i=0;i<kamers1.object.getQuestionCount();i++) {
                ...
                String text= scanner.nextLine();
                checker.sendCorrectionMessage(Counter, text);
                Counter++;
            }
            System.out.println();
